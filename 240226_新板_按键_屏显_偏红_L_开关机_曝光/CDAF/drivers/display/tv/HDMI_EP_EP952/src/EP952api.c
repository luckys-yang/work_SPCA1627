...
#if 0	//for 1628 IC
	//gpioByteFuncSet(GPIO_BYTE_GEN2, 0x01 << (23 & 0x07), 0xff);
	gpioByteOutSet(GPIO_BYTE_GEN2, 0x01 << (23 & 0x07), 0x00);
	gpioByteDirSet(GPIO_BYTE_GEN2, 0x01 << (23 & 0x07), 0xff);
	HAL_GlobalTimerWait(20);
	gpioByteOutSet(GPIO_BYTE_GEN2, 0x01 << (23 & 0x07), 0xff);
	HAL_GlobalTimerWait(10);
#else	//for 1626 IC
	gpioByteFuncSet(GPIO_BYTE_GEN0, 0x01 << (6 & 0x07), 0xff);
	gpioByteOutSet(GPIO_BYTE_GEN0, 0x01 << (6 & 0x07), 0x00);
	gpioByteDirSet(GPIO_BYTE_GEN0, 0x01 << (6 & 0x07), 0xff);
	HAL_GlobalTimerWait(20);
	gpioByteOutSet(GPIO_BYTE_GEN0, 0x01 << (6 & 0x07), 0xff);
	HAL_GlobalTimerWait(10);
#endif
...
void  EP_HDMI_Set_Video_Timing(unsigned char VIC)
{
		...
		//DK[3:1]=110,DKEN=1,DSEL=0,BSEL=0,EDGE=1,FMT12=1
		EP952C_Registers.Video_Interface[0] = 0x83;/*CCIR601*/	// reference to Video_Interface[0].jpg;

		#if DISP_CCIR601_PROGRESSIVE
		//SYNC[3:2]=01,VIN_FMT[1:0]=0b10 FOR CCIR601
		EP952C_Registers.Video_Interface[1] = 0x06;	// reference to Video_Interface[1].jpg;
		#endif

		#else

		//DK[3:1]=110,DKEN=1,DSEL=0,BSEL=1,EDGE=1,FMT12=0
		EP952C_Registers.Video_Interface[0] = 0xd6;	// reference to Video_Interface[0].jpg;

		//SYNC[3:2]=0,VIN_FMT[1:0]=0b00
