...
#define ABS(x, y) ((x) > (y)) ? ((x) - (y)) : ((y) - (x))   // 获得他们绝对差值
...
static bpPara_t code sBpPara =
{
    ...
    30,   // 阈值，用于某些图像处理算法中判断坏点的标准
    40,   // 修改后的阈值，原注释：30，表示此参数被更新为40

    BADPIXEL_PV_NUM_MAX, // 预览时坏点的最大数量，原注释：4096
    BADPIXEL_PV_NUM_MAX, // 预览时坏点的最大数量，保持一致，原注释：4096
    BADPIXEL_CAP_NUM_MAX - 4, // 捕获（拍照）时坏点的最大数量减去4，原注释：8000

    0,    // 保留字段，当前未使用，设置为0
    0,    // 保留字段，当前未使用，设置为0

    0x55aa, // 文件结束标志EOF（End Of File），用于标识结构体定义结束
};
...
* @param    None
* @retval   None
* @brief    进行应用程序校准配置的初始化
**/
void appCalInit(void)
{
    appAutoOffEn(0); // 禁用自动关机功能。参数0通常表示禁用。

    sp1kCalibrationLoadCriteria(SP1K_CALIBRATION_CRITERIA_FILE_ID); // 从指定的文件ID中读取自动曝光测试参数，这些参数用于后续的校准过程。

    sp1kCalibrationAeTestGidStepGet(id, id + 1, id + 2, id + 3); // 获取并保存自动曝光测试的步骤。这里假设`id`等变量已经在某处定义，用于指定具体的测试参数或步骤。

    sp1kCalibrationDefaultSizeSet(); // 初始化校准的默认尺寸。这可能意味着为图像校准过程设置一个默认的分辨率或大小。

    appCalCfgDataRst(); // 重置校准配置数据，可能是将所有校准相关的配置恢复到初始状态或默认值。
}

/**
* @param    None
* @retval   None
* @brief    设置拍照参数为保存原始图像，播放拍照提示语音，然后执行拍照任务
**/
void appCalSnapRaw(void)
{
    // 设置拍照参数为保存原始图像
...
* @param    None
* @retval   None
* @brief    执行IQ RAW拍照，包括设置校准参数、拍照和恢复参数等操作
**/
void appCalSnapIQRaw(void)
{
    ...
    /* 恢复OB, LSC, AWB, Gamma校正，启用CDSP后处理，设置AE模式，启用AWB */
    sp1kCalibrationObSet(puiPara->cal_ob);
    sp1kCalibrationLscSet(puiPara->cal_lc);
    sp1kCalibrationAWBSet(puiPara->cal_awb);
    sp1kCalibrationGammaSet(puiPara->cal_gma);
...
void appCardUpdateFW(void)
{
    UINT8 strwidth = 0, strheight = 0; // 定义字符串宽度和高度变量
    UINT8 xpos, ypos; // 定义x和y坐标变量
    UINT8 str[30] = "ISP"; // 定义一个字符串数组并初始化为"ISP"

    code UINT8  filestr[20] = "D:\\1628.BIN", file1str[30] = "D:\\DCIM\\100MEDIA\\"; // 定义两个只读字符数组，用于存储文件路径
    UINT8 fd = DOS_FileOpen(filestr, 'r'); // 打开文件

    dbgWdtSet(0); // 设置调试看门狗定时器

    if (!fd) // 如果文件未打开成功
    {
#if SUPPORT_AUTO_TEST   // 【自动测试功能】
        UINT8 en; // 定义一个变量用于临界区保护
#endif

#if SUPPORT_AUTO_TEST   // 【自动测试功能】
        // 进入临界区保护
        ENTER_CRITICAL(en);
        fd = DOS_FileOpen(SUPPORT_AUTO_TEST_FILENAME, 'r'); // 打开自动测试文件
        if (fd)
        {
            autoTestOpenFlag = 1; // 设置自动测试打开标志
        }
        DOS_FileClose(fd); // 关闭文件
        // 退出临界区保护
        EXIT_CRITICAL(en);
#endif

        DIR_Change(file1str); // 切换目录至指定路径
        return; // 函数返回
    }
    DOS_FileClose(fd); // 关闭文件

    app_set_led(1); // 设置LED指示灯状态
    appFlashLedSet(1); // 设置闪光LED状态

    app_set_lcd_backlight_status(1); // 设置LCD背光状态

#if SUPPORT_SPI_WRITE_CONTROL
    mastWriteSpiFlag = 1; // 设置SPI写控制标志
    appSpiWpSet(0); // 设置SPI写保护
    sp1kHalWait(300); // 等待一段时间
#endif

    sp1kOsdIndexSave(); // 保存OSD索引
    sp1kOsdPenCfg(OSD_ATTR_MODE_BLD | OSD_ATTR_COLOR_BLACK, 4); // 配置OSD画笔
    sp1kOsdRectDraw(0, 0, 320 , 240); // 绘制矩形
    sp1kOsdIndexRestore(); // 恢复OSD索引

    strGet(str, sizeof(str), ID_STR_SOFTWARE_UPDATE___); // 获取字符串
    xpos = (320 - strWidthGet(str)) >> 1; // 计算字符串的x坐标位置
    ypos = (240 - 32) >> 1; // 计算字符串的y坐标位置
    sp1kOsdStrDrawExt(xpos, ypos, str, 1); // 绘制扩展字符串
    strGet(str, sizeof(str), ID_STR_WAITING___); // 获取字符串
    sp1kOsdStrDrawExt(xpos, ypos + 32, str, 1); // 绘制扩展字符串

    sp1kHalWait(100); // 等待一段时间
    if(sp1kCardFirmwareUpdate(filestr , 0) == FAIL) // 如果卡固件更新失败
    {
        sp1kOsdIndexSave(); // 保存OSD索引
        sp1kOsdPenCfg(OSD_ATTR_MODE_BLD | OSD_ATTR_COLOR_BLACK, 4); // 配置OSD画笔
        sp1kOsdRectDraw(0, 0, 320 , 240); // 绘制矩形
        ...
        strGet(str, sizeof(str), ID_STR_UPDATE_FAILED_); // 获取字符串
        xpos = (320 - strWidthGet(str)) >> 1; // 计算字符串的x坐标位置
        ypos = (240 - 32) >> 1; // 计算字符串的y坐标位置
        sp1kOsdStrDrawExt(xpos, ypos, str, 1); // 绘制扩展字符串

        app_set_led(0); // 设置LED指示灯状态
        sp1kHalWait(200); // 等待一段时间
        app_set_led(1); // 设置LED指示灯状态
        sp1kHalWait(200); // 等待一段时间
        app_set_led(0); // 设置LED指示灯状态
    }
    else // 如果卡固件更新成功
    {
        ...
        strGet(str, sizeof(str), ID_STR_UPDATE_COMPLETED_); // 获取字符串
        xpos = (320 - strWidthGet(str)) >> 1; // 计算字符串的x坐标位置
        ypos = (240 - 32) >> 1; // 计算字符串的y坐标位置
        sp1kOsdStrDrawExt(xpos, ypos, str, 1); // 绘制扩展字符串

        appPowerShutDown(); // 关闭电源
    }

#if SUPPORT_SPI_WRITE_CONTROL
    mastWriteSpiFlag = 0; // 清除SPI写控制标志
#endif
}
...
{
    UINT16 frontHSize, frontVSize;  // 定义前置预览尺寸变量
    UINT16 cropHSize, cropVSize;    // 定义裁剪尺寸变量
    UINT8 En = 1;       // 启用标志
    UINT8 i;
    UINT16 afHShift, afVShift, afHStart, afHEnd, afVStart, afVEnd;
...
#if SUPPORT_AWB_ADJ
    awb_adj_start_flag = 0; // 支持自动白平衡调整
#endif

    averageAfValue = 0; // 平均自动对焦值初始化
    afValueArrayIdx = 0;    // 自动对焦值数组索引初始化
    for (i = 0; i < AF_ONE_D_FILTER; i++)
    {
        afValueArray[i] = 0;
    }

    sp1kFrontPreviewSizeGet(0, &frontHSize, &frontVSize);   // 获取前置预览尺寸
    sp1kPreviewCropSizeGet(&cropHSize, &cropVSize); // 获取裁剪尺寸

    sp1kAfWinCfg(frontHSize, frontVSize, cropHSize, cropVSize); // 配置自动对焦窗口

    sp1kAfWinEnSet(En); // 设置自动对焦窗口启用
    ...
    /* 基于320*240比率，320*180比率 */
    AFWINDOW[i].xpos = afHStart / (cropHSize / 320);
    AFWINDOW[i].ypos = afVStart / (cropHSize / 320);
    AFWINDOW[i].width = (cropHSize - afHEnd) / (cropHSize / 320);
    AFWINDOW[i].heigth = (cropVSize - afVEnd) / (cropHSize / 320);

    sp1kTimer1Config(16, pFunc_AfCal, NULL);    // 配置定时器1
